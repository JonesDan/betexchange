        ...
        for market_book in market_books:
            ...
            if 'rc' in data:
                for d in data['rc']:
                    if 'batl' in d:
                        for d1 in d['batl']:
                            summary = {'market_id':data['id'],'selection_id':d['id'] ,'level':d1[0], 'price':d1[1] ,'size':d1[2],'b_l':'LAY', 'update_time':time}
                            datalist.append(summary)
                    
                    if 'batb' in d:
                        for d2 in d['batb']:
                            summary = {'market_id':data['id'],'selection_id':d['id'] ,'level':d2[0], 'price':d2[1] ,'size':d2[2],'b_l':'BACK', 'update_time':time}
                            datalist.append(summary)

            for d in datalist:
                print(d)
            # sse.publish(data, type='update')  # Publish data to SSE

stream_price('33925128')