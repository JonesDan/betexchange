...
def calc_order_exposure():
    query_sqlite(f"""
                CREATE TABLE selection_exposure AS
                    WITH back_orders AS (
                        SELECT 
                        market_id,
                        selection_id,
                        price,
                        size,
                        price * size AS back_winnings,
                        (price * size) - size AS back_profit
                        FROM 
                        orders
                        WHERE side = 'BACK'
                          ),
                    WITH lay_orders AS (
                        ...
                        price,
                        size,
                        price * size AS lay_winnings,
                        (price * size) - size AS lay_liability,
                        size AS lay_profit
                        FROM 
                        orders
                        WHERE side = 'LAY'
                    ...
                    SELECT
                        COALESCE(b.market_id, l.market_id) AS market_id,
                        COALESCE(b.selection_id, l.selection_id) AS selection_id,
                        SUM(b.back_profit - l.lay_liability) AS back_exposure,
                        SUM(b.lay_profit - l.back_size) AS lay_exposure,
                        SUM((b.back_profit - l.lay_liability) - (b.lay_profit - l.back_size)) AS exposure
                    FROM
                        back_orders b
                    FULL OUTER JOIN lay_orders l
                        ON b.market_id = l.market_id AND b.selection_id = l.selection_id
                    """)
...
def init_db():
    with sqlite3.connect('mydata.db') as conn:
        ...
        for table in sqlite_tables:
            ...
            cursor.execute(f'CREATE TABLE IF NOT EXISTS market_catalogue ({sqlstring})')
        conn.commit()
...
def list_market_catalogue(trading, event_id):
    ...
    for market in market_catalogue:
        ...
        for x in market.runners:
            data = {'key': f"{market['market_id']}_{market['selection_id']}", 'market_name':market_name, 'market_id': market_id, 'start_time': start_time, 'selection_name': x.runner_name, 'selection_id': x.selection_id, 'event': event, 'total_matched': total_matched, 'desc': desc}
            market_catalogue_list.append(data)
            upsert_sqlite(sqlite_tables['market_catalogue'], data)
...
def upsert_sqlite(table, data):
    ...
    values1 = ', '.join(['?' for col in sqlite_tables[table] if col['name'] in data])
    values2 = ', '.join(data[col['name']] for col in sqlite_tables[table] if col['name'] in data)
    with sqlite3.connect('mydata.db') as conn:
        cursor = conn.cursor()
        cursor.execute(f'REPLACE INTO data ({columns}) VALUES ({values1})', ({values2}))
        conn.commit()
