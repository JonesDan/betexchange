...
from utils import init_logger, bf_login, prices_listener, orders_listener, clear_shelve, place_order, list_market_catalogue, list_events, orders_agg, list_current_orders, upsert_sqlite, dict_factory, query_sqlite
import shelve
import queue
...
def eventDetail(event_id):
    try:
        ...
        market_ids_list = query_sqlite("SELECT DISTINCT event, market_id, market_name, printf('%.2f', total_matched) FROM market_catalogue")

        # conn = sqlite3.connect('your_database.db')
        # conn.row_factory = dict_factory
        # cursor = conn.cursor()

        # cursor.execute("SELECT DISTINCT column1, column2, column3 FROM your_table")
        ...
        #     # summary = {'market_id': market['market_id'],'selection_id': market['selection_id'], 'selection_name':market['selection_name'], 'market_name':market['market_name']}
        #     summary = {'key': f'{market['market_id']}_{market['selection_id']}'} | market_data
        #     upsert_sqlite(sqlite_tables['market_catalogue'], summary)
...
def get_prices():
    ...
    prices = query_sqlite(f"""
                          ...
                          FROM 
                            price p JOIN market_catalogue m
    ...
    for order in order_summary.keys():
        if order_summary[order]['market_id'] in market_selections:
