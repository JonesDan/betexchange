    ...
    with app.app_context():
        ...
        for message in pubsub.listen():
            if message['type'] == 'message':
                ...
                levels = list(data_cache[market_id][selection_id][b_l].keys())
                size = [data_cache[market_id][selection_id][b_l][level]['size'] for level in levels]
                id = '-'.join(data['id'].split('-')[:-1])

                data2 = {'levels':levels, 'size': size, 'id':id}

                sse.publish(data2, type='update')
...
def event_detail(event_id):
    ...
    return render_template('index.html', markets=market_catalogue_distinct)
